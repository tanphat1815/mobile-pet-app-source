...
### Step M-14 (Build config - EAS + iOS/Android)
- `eas.json`:
  - `cli.version >= 14.0.0`, `appVersionSource: remote`
  - `build.base` shared settings: Node 20.11.1, Yarn 1.22.22,
    EXPO_PUBLIC_* env vars (API_BASE_URL, WS_URL, ENV)
  - `build.base.ios`: latest image, bundleIdentifier
    `com.mobilepet.app`, simulator off, enterpriseProvisioning
    never
  - `build.base.android`: gradle 8.10.2, latest image,
    buildType app-bundle
  - `build.development`: development client on, internal
    distribution, channel development, env -> localhost (with
    simulator override for `eas build --profile simulator`)
  - `build.preview`: internal distribution, channel preview,
    env -> staging URLs (https://staging.mobilepet.app)
  - `build.production`: autoIncrement on, channel production,
    env -> production URLs (https://api.mobilepet.app)
  - `build.simulator`: extends development with ios.simulator
  - `submit.production` / `submit.preview`: App Store + Play
    Store placeholders for ASC app identifier, Apple team ID,
    and Play Store service-account JSON key path
- `app.json`:
  - `scheme: mobilepet`
  ...
  - `extra.appName`, `extra.bundleIdentifier`,
    `extra.androidPackage` for build-time resolution
  - iOS: `associatedDomains: ["applinks:mobilepet.app"]`,
    `usesUniversalLinks: true`,
    `infoPlist.CFBundleDisplayName`,
    `CFBundleShortVersionString`, `CFBundleVersion`
  - Android: `versionCode`, additional `VIBRATE` permission
- `scripts/bump-version.js`:
  - Atomic version bumper (`patch | minor | major`)
  - Updates `package.json`, `app.expo.version`,
    `app.expo.ios.infoPlist.CFBundleVersion` (string),
    `app.expo.ios.infoPlist.CFBundleShortVersionString`,
    `app.expo.android.versionCode`
- `.gitignore`: build artifacts (build/, dist/, .expo/, *.apk,
  *.aab, *.ipa, *.app), env files, secrets/, native projects
  generated by `expo prebuild`, etc.
- `docs/eas-build.md`: 10-step guide covering
  - Install EAS CLI / eas login / eas init
  - Apple App Store / Google Play Store config placeholders
  - All four build profiles (`development`, `preview`,
    `production`, `simulator`)
  - `eas submit` flow (iOS / Android)
  - Environment variable matrix
  - Secrets storage policy
  - `npm run prebuild` flow
  - `npm run version:patch|minor|major` flow
- `package.json` scripts:
  - `prebuild` -> `expo prebuild --no-install`
  - `typecheck` -> `tsc --noEmit`
  - `build:dev` / `build:preview` / `build:production` /
    `build:simulator` (EAS)
  - `submit:production` / `submit:android:production`
  - `version:patch` / `version:minor` / `version:major`
- `src/utils/runtimeConfig.ts`: typed env reader
  - Reads `EXPO_PUBLIC_*` env vars at runtime
  - Exports `ENV` (`development` | `staging` | `production` |
    `unknown`), `IS_DEVELOPMENT_BUILD`,
    `IS_STAGING_BUILD`, `IS_PRODUCTION_BUILD`, `IS_PROD_LIKE`
  - Exports `RUNTIME_API_BASE_URL`, `RUNTIME_WS_URL`,
    `RUNTIME_BUILD_NUMBER`, `RUNTIME_APP_VARIANT`,
    `USE_DEVELOPMENT_CLIENT`
  - Exports `envLabel()` and `logRuntimeConfig()`
- `src/api/client.ts`: prefers `RUNTIME_API_BASE_URL` when
  defined, otherwise falls back to the `api/config.ts` default
- `src/api/SyncManager.ts`: prefers `RUNTIME_WS_URL` when
  defined, otherwise falls back to `WS_URL`
- `App.tsx`: calls `logRuntimeConfig()` once on mount
